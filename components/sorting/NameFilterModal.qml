...
Item {
    ...
    Rectangle {
        ...
        Footer.Component {
            ...
            // Only the order differs: Xbox (the default - see
            // settings/Handler.qml) leads with Accept, Nintendo-style pads
            // lead with Cancel. The printed letters come from
            // theme.buttonGuide either way, which already handles X and Y
            // being swapped between the two styles (see SwitchButtons.qml)
            buttons: theme.xboxMode ? [
                { title: 'Search', key: theme.buttonGuide.accept, square: false, sigValue: 'accept' },
