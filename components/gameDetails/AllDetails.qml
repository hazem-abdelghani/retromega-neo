...
Item {
    ...
    property string imgSrc: {
        ...
        // see ScreenshotPreview.qml - an absent asset can come back undefined
        return currentGame.assets.screenshot ?? '';
    }

    // named so it can be handed back to removeCallback() below
    function gameDetailsVideoCallback() { gameDetailsVideo.switchVideo(); }

    Component.onCompleted: {
        gameDetailsVideo.switchVideo();
        settings.addCallback('gameDetailsVideo', gameDetailsVideoCallback);
    }

    Component.onDestruction: {
        settings.removeCallback('gameDetailsVideo', gameDetailsVideoCallback);
    }
